...
namespace objl
{
	...
	public:
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if ((outputIndicator = ((outputIndicator + 1) % outputEveryNth)) == 1)
				{
					if (!meshname.empty())
					{
                        ...
                        if (flag)
                        {
                            ss << "Loading .obj file " << meshname;
                            flag = false;
                        }
                            ss << "\n\t vertex count: " << Positions.size()
                            << "\n\t texcoord count " << TCoords.size()
                            << "\n\t normal count " << Normals.size()
                            << "\n\t triangle count " << (Vertices.size() / 3);

                        /*if (!MeshMatNames.empty())
                            ss << "\t| material: " << MeshMatNames.back();*/
				...
				if (algorithm::firstToken(curline) == "mtllib")
				{
					...
					#ifdef OBJL_CONSOLE_OUTPUT
                    std::stringstream ss;
                    ss << "Loading materials at: " << pathtomat;
					//CH_TRACE(ss.str());
					#endif
            ...
            {
                std::stringstream ss;
                ss << "Loaded model " << meshname << " at " << Path
                    << "\n\t vertex count: " << Positions.size()
                    << "\n\t texcoord count: " << TCoords.size()
                    << "\n\t normal count: " << Normals.size()
                    << "\n\t triangle count: " << (Vertices.size() / 3);
