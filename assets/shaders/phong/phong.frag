...
void main(){
    ...
    float specular = 0.0;
    if (lambertian > 0.0) {
        vec3 R = reflect(-L, N)/*normalize(2 * dot(N, L) * N - L)*/;    // Reflected light vector
        vec3 V = normalize(u_CameraPos - frag_Pos);     // Vector to viewer
        // Compute the specular term
        float specAngle = max(dot(R, V), 0.0);
        specular = pow(specAngle, u_Material.shininess);
    }

    float fatt = 1.0 / (1.0 + 0.02 * lDistance + 0.0004 * lDistance * lDistance);
	vec4 fColor;
